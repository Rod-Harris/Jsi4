...
public abstract class Task
{
	...
	*/
	...
	protected EString log = new EString();
	...
	*/
	protected abstract void do_try() throws Throwable;
	...
	*/
	protected void do_catch( Throwable error )
	{
	}
	...
	*/
	private final void do_finally( Throwable error )
	{
	}
	...
	*   runs this tasks do_try() (and do_catch() if do_try() threw an exception) and do_finally() once
	*/
	public void do_unsafe() throws Throwable
	{
		do_unsafe( 1 );
	}
	...
	*/
	public void do_safe( int max_loops )
	{
		...
		while( true )
		{
			...
			{
				cerr.println( "Error: %s do_try() method has thrown the error:", this );
				...
				{
					cerr.println( "Error unsafe do_catch method in %s has thrown the error:", this );

					cerr.println( retrace( error2 ) );
				}
			}
			finally
			{
				...
				{
					cerr.println( "Error unsafe do_finally method in %s has thrown the error:", this );
	...
	*/
	public void do_unsafe( int max_loops ) throws Throwable
	{
		...
		while( true )
		{
			...
			catch( Exception ex ) // catches checked and unchecked exceptions, but not errors
			{
				...
				if( loops == this.max_loops ) throw ex;
			}
			finally
			{
				try

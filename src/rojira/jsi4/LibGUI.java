...
public class LibGUI
{
	...
	public static void set_colour( Graphics2D g2d, int colour )
	{
		g2d.setColor( new Color( argbR( colour ), argbG( colour ), argbB( colour ), argbA( colour ) ) );
	}

	public static String aarrggbb_hex( int colour )
	{
		return fmt( "#%x", colour );
	}


	public static String rrggbb_hex( int colour )
	{
		return "#" + fmt( "%x", colour ).substring( 2 );
	}
...
*/
		BufferedImage new_image = new BufferedImage( new_width, new_height, image.getType() );
		
		Graphics2D g = new_image.createGraphics();
		
		g.setRenderingHints( quality_image_hints );
...
*/
	...
	public static BufferedImage shade_image( BufferedImage image, ImageShader shader )
	{
		return shader.apply( image );
	}


	public static BufferedImage filter_image( BufferedImage image, ImageShader shader )
	{
		return shader.apply( image );
	}
...
//			println( "mask_image error: images are not the same size" );
	...
	public static BufferedImage[] extract_tiles( BufferedImage image, int xtiles, int ytiles )
	{
		return extract_tiles( image, xtiles, ytiles, true );
	}


	public static BufferedImage[] extract_tiles( BufferedImage image, int xtiles, int ytiles, boolean row_major )
	{
		check_arg( image.getWidth() % xtiles == 0, "images of width %d can't be divided into %d tiles", image.getWidth(), xtiles );

		check_arg( image.getHeight() % ytiles == 0, "images of height %d can't be divided into %d tiles", image.getHeight(), ytiles );

		BufferedImage[] images = new BufferedImage[ xtiles * ytiles ];

		int tw = image.getWidth() / xtiles;

		int th = image.getHeight() / ytiles;
		...
		if( row_major )
		{
			for( int x=0; x<xtiles; x++ )
			{
				for( int y=0; y<ytiles; y++ )
				{
					...
					images[ i ] = get_image_block( image, x0, y0, x1, y1 );
	...
	*/
	...
	public static BufferedImage flip_image( BufferedImage image )
	{
